...
/// <summary>
/// Localized blur brush. Stamps a circular dab that averages neighboring pixels
/// within a small kernel, blending the blurred result into the destination
/// using brush falloff, flow, and opacity.
/// </summary>
public sealed class BlurBrushTool : ITool
{
    public string Name => "Blur";
    public string IconResourceKey => "IconBlur";
    ...
    /// <summary>Blur strength from 0.0 to 1.0 (controls kernel size and blend amount).</summary>
    ...
    private void StampDab(Point center)
    {
        ...
        // Kernel radius scales with strength: 1 (3×3) to 3 (7×7)
        int kernelRadius = Math.Max(1, (int)Math.Round(Strength * 3));
        ...
        for (int y = yMin; y <= yMax; y++)
        {
            ...
            for (int x = xMin; x <= xMax; x++)
            {
                ...
                // Compute average of neighboring pixels within the kernel
                int count = 0;
                sums.Clear();
                ...
                for (int c = 0; c < channels; c++)
                {
                    ushort averaged = (ushort)(sums[c] / count);
                    ushort original = row[offset + c];
                    row[offset + c] = (ushort)(original + (averaged - original) * alpha);
                }
            }
        }
    }
